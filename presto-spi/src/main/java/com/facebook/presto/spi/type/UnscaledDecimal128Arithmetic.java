...
public final class UnscaledDecimal128Arithmetic
{
    ...
    /**
     * multiply two decimals. This method never throws an overflow exception
     * but instead always return exact result (256bit - array of 8 integers)
     */
    public static int[] multiplyWithoutOverflow(Slice left, Slice right)
    {
        int l0 = getInt(left, 0);
        int l1 = getInt(left, 1);
        int l2 = getInt(left, 2);
        ...
        product = (r0 & LONG_MASK)
                * (l0 & LONG_MASK);
        result[0] = (int) product;

        //TODO: fix overflow bug
        product = (r0 & LONG_MASK)
                * (l1 & LONG_MASK)
                + (r1 & LONG_MASK)
                * (l0 & LONG_MASK) + (product >>> 32);
        result[1] = (int) product;
        ...
        product = (r0 & LONG_MASK)
                ...
                * (l1 & LONG_MASK)
                + (r3 & LONG_MASK)
                * (l0 & LONG_MASK) + (product >>> 32);
        result[3] = (int) product;

        product = (r1 & LONG_MASK)
                * (l3 & LONG_MASK)
                + (r2 & LONG_MASK)
                * (l2 & LONG_MASK)
                + (r3 & LONG_MASK)
                * (l1 & LONG_MASK) + (product >>> 32);
        result[4] = (int) product;

        product = (r2 & LONG_MASK)
                * (l3 & LONG_MASK)
                + (r3 & LONG_MASK)
                * (l2 & LONG_MASK) + (product >>> 32);
        result[5] = (int) product;

        product = (r3 & LONG_MASK)
                * (l3 & LONG_MASK) + (product >>> 32);
        result[6] = (int) product;
        result[7] = (int) (product >>> 32);
    ...
    static void shiftLeftDestructive(Slice decimal, int leftShifts)
    {
        ...
        // check overflow
        if (bitShiftsInWord != 0) {
            if ((getLong(decimal, 1 - wordShifts) >> shiftRestore) != 0) {
                throwOverflowException();
            }
        }
        if (wordShifts == 1) {
    ...
    {
        int[] multiplicationResult = multiplyWithoutOverflow(decimal, multiplier);
        shiftRightArray8(multiplicationResult, rightShifts, decimal);
    }
